...
<table width="100%" style="margin-top: 10px; font-size: 13px; border-collapse: collapse;">
    <thead style="border-bottom: 2px solid black;">
        <tr style="font-weight: bold;">
            <th style="width: 40%;">বিষয় / <span style="font-size: 11px;">Subject</span></th>
            <th>বাংলা</th>
            <th style="font-size: 11px;">English</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>প্রতিষ্ঠানের নাম / <span style="font-size: 11px;">Name of Organization</span></td>
            <td>{{ $rowbn->applicant_name_of_the_organization }}</td>
            <td style="font-size: 11px;">{{ $row->applicant_name_of_the_organization }}</td>
        </tr>
        <tr>
            <td>লাইসেন্সধারীর নাম / <span style="font-size: 11px;">Licensee Name</span></td>
            <td>{{ $rowbn->applicant_name }}</td>
            <td style="font-size: 11px;">{{ $row->applicant_name }}</td>
        </tr>
        <tr>
            <td>পিতা/স্বামীর নাম / <span style="font-size: 11px;">Father’s or Husband’s Name</span></td>
            <td>{{ $rowbn->applicant_father_name }}</td>
            <td style="font-size: 11px;">{{ $row->applicant_father_name }}</td>
        </tr>
        <tr>
            <td>মাতার নাম / <span style="font-size: 11px;">Mother’s Name</span></td>
            <td>{{ $rowbn->applicant_mother_name }}</td>
            <td style="font-size: 11px;">{{ $row->applicant_mother_name }}</td>
        </tr>
        <tr>
            <td>ব্যবসার প্রকৃতি / <span style="font-size: 11px;">Nature of Business</span></td>
            <td>{{ $rowbn->applicant_owner_type }}</td>
            <td style="font-size: 11px;">{{ $row->applicant_owner_type }}</td>
        </tr>
        <tr>
            <td>ব্যবসার ধরন / <span style="font-size: 11px;">Type of Business</span></td>
            <td>{{ $rowbn->applicant_type_of_business }}</td>
            <td style="font-size: 11px;">{{ $row->applicant_type_of_business }}</td>
        </tr>
        <tr>
            <td>প্রতিষ্ঠানের ঠিকানা / <span style="font-size: 11px;">Organization Address</span></td>
            <td>{{ $rowbn->organization_address }}</td>
            <td style="font-size: 11px;">{{ $row->organization_address }}</td>
        </tr>
        <tr>
            <td>ওয়ার্ড নং / <span style="font-size: 11px;">Word No.</span></td>
            <td>{{ int_en_to_bn($rowbn->applicant_present_word_number) }}</td>
            <td style="font-size: 11px;">{{ $row->applicant_present_word_number }}</td>
        </tr>
        <tr>
            <td>জাতীয় পরিচয়পত্র নং / <span style="font-size: 11px;">NID No.</span></td>
            <td>{{ int_en_to_bn($rowbn->applicant_national_id_number) }}</td>
            <td style="font-size: 11px;">{{ $row->applicant_national_id_number }}</td>
        </tr>
        <tr>
            <td>অর্থবছর / <span style="font-size: 11px;">Fiscal Year</span></td>
            <td>{{ int_en_to_bn($rowbn->orthoBchor) }}</td>
            <td style="font-size: 11px;">{{ $row->orthoBchor }}</td>
        </tr>
    </tbody>
</table>
...
@endphp
    ...
    <table width='100%' style="font-size: 12px">
        ...
        <td width='50%' align ="right">
            ...
            @if($row->hasEnData==1)
            <b style="color:black">সর্বমোট : {{ int_en_to_bn(($rowbn->currently_paid_money)+$amount_deails->last_years_money ?? 0) }} টাকা মাত্র </b>
            @else
            <b style="color:black">সর্বমোট : {{ int_en_to_bn($rowbn->currently_paid_money+$amount_deails->last_years_money ?? 0) }} টাকা মাত্র </b>
            @endif
